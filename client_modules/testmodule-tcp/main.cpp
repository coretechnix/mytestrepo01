...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
    	// ./evtdistr_test_module_tcp.bin -h127.0.0.1 -p4599 -cM2 -tstart -mMEDIA-001
        ...
        c = getopt_long(argc, argv,"h:p:c:t:m:",long_options,&option_index);
        if (c == -1)
            break;
        ...
        switch (c)
        {
            ...
            case 'h':
                    ...
                    host.assign( std::string(optarg) );
                    break;
            case 'p':
                    //printf("option p (port):'%s'\n",optarg);
                    port = atoi( optarg );
                    break;
            case 'c':
    ...
    printf("Connecting to '%s:%d', Message gen. params: chid: '%s', event type: '%s', media item: '%s'\n",host.c_str(),port,chid.c_str(),type.c_str(),media_item.c_str());
