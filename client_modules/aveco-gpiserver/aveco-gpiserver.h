...
class ths_unordered_map
{
    ...
    public:
            ...
            {
                // Because unordered_map containers do not allow for duplicate keys, this
                // means that the function actually returns 1 if an element with that
                // key exists in the container, and zero otherwise.
                std::lock_guard<std::mutex> _(mtx);
...
typedef System::Config::Event::Distr::Server::Modul::Aveco::GPI::Config config_t;

class AvecoGPIServer
{
    private:
            ...
            std::vector<std::thread>& get_threads(); // ### must fix this - returning ref is not OK
            ...
            bool parse_pbmsg_from_codedstream(system_msg_t &_msg, const char *_ackBuf, const int &_bufsize);
    public:
            std::error_code ec;
            AvecoGPIServer();
            ~AvecoGPIServer();
